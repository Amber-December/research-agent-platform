...
- **`/rebuttal`:** parses reviewer comments, pauses only for genuine response-strategy choices, drafts point-by-point responses, and creates a revision plan.
...
- **Drag-and-drop uploads:** `POST /api/session/files` sanitizes names, enforces file count/size limits, classifies files into workspace directories, records upload batches, and allows `/present` to freeze the latest batch.
